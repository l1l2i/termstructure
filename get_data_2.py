...
def process_futures(engine):
    futures_data = get_futures(currency=CURRENCY)
    index_price = get_index_price(currency=CURRENCY)
    records = []
    for future in futures_data['result']:
        order_book = get_book(instrument=future['instrument_name'], depth=DEPTH)
        #order_book = get_book(instrument='ETH-5JAN24', depth=5)
        spot_book = get_book(instrument=SPOT, depth=DEPTH)
        ...
        ask_sizes_fut = sum([ask[1] for ask in order_book['result']['asks'][:DEPTH]])
        bid_sizes_fut = sum([bid[1] for bid in order_book['result']['bids'][:DEPTH]])
        ask_sizes_spot = sum([ask[1] for ask in order_book['result']['asks'][:DEPTH]])
        bid_sizes_spot = sum([bid[1] for bid in spot_book['result']['bids'][:DEPTH]])
        market_delta_fut =  bid_sizes_fut - ask_sizes_fut
        market_delta_spot = bid_sizes_spot - ask_sizes_spot  
